...
class CustomUser(AbstractUser, PermissionsMixin):
    ...
    def enable_two_factor(self):  # Método mejorado para habilitar 2FA
        """ Habilitar 2FA para el usuario """
        if not self.two_factor_enabled:
            device = TOTPDevice.objects.get_or_create(user=self)[0]
            device.generate_challenge()  # Genera un nuevo código OTP
            self.two_factor_enabled = True
            self.save()
            print("Código OTP:", device.generate_challenge())
    ...
    def generate_two_factor_code(self):
        ...
        print(f"Código 2FA generado: {self.two_factor_code}")
        self.save()
